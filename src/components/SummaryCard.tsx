...
export function SummaryCard({ session }: SummaryCardProps) {
  ...
  return (
    <div className="bg-background-secondary border border-border-primary rounded-lg p-6 mb-6">
      <h3 className="text-xl font-semibold text-text-primary mb-4">Session Summary</h3>

      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
        {/* Total Cost */}
        <div>
          <p className="text-text-tertiary text-sm mb-1">Total Cost</p>
          <p className="text-accent-cost text-2xl font-bold">{formatCost(session.totalCost)}</p>
        </div>

        {/* Message Counts */}
        <div>
          <p className="text-text-tertiary text-sm mb-1">Messages</p>
          <p className="text-text-primary text-2xl font-bold">{formatNumber(session.messageCount)}</p>
        ...
        {/* Tokens */}
        <div>
          <p className="text-text-tertiary text-sm mb-1">Tokens</p>
        ...
        {/* Cache Hit Rate */}
        <div>
          <p className="text-text-tertiary text-sm mb-1">Cache Hit Rate</p>
          <p className="text-text-primary text-2xl font-bold">{cacheHitRate}</p>
          <p className="text-text-tertiary text-xs mt-1">
        ...
        <div>
          ...
          <p className="text-text-primary text-2xl font-bold">{duration}</p>
        </div>
      </div>

      {/* Additional Token Breakdown */}
      <div className="mt-4 pt-4 border-t border-border-primary">
        <p className="text-text-tertiary text-sm mb-2">Token Breakdown</p>
        <div className="flex gap-4 text-sm">
          <div>
            <span className="text-text-tertiary">Input: </span>
            <span className="text-text-primary font-mono">{formatNumber(tokenStats.inputTokens)}</span>
          </div>
          <div>
            <span className="text-text-tertiary">Cache Write: </span>
            <span className="text-text-primary font-mono">{formatNumber(tokenStats.cacheWriteTokens)}</span>
          </div>
          <div>
            <span className="text-text-tertiary">Cache Read: </span>
            <span className="text-text-primary font-mono">{formatNumber(tokenStats.cacheReadTokens)}</span>
          </div>
          <div>
            <span className="text-text-tertiary">Output: </span>
            <span className="text-text-primary font-mono">{formatNumber(tokenStats.outputTokens)}</span>
          </div>
        </div>
      </div>
    </div>
